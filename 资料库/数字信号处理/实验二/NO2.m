...
for Fc = linspace(Fc_start, Fc_end, 20) 
    ...
    while true
        ...
        % 将增益转换为dB并检查是否满足衰减要求
        if (20*log10(gain_f1) >= -3 && 20*log10(gain_f2) >= -3 && 20*log10(gain_f3) <= -50 && 20*log10(gain_f4) <= -50)
            % 找到满足要求的滤波器
            if N < min_N
                min_N = N;
                optimal_N = N;
...
fprintf('最小满足要求的滤波器阶数N为：%d，对应的截止频率Fc为：%.2f Hz\n', optimal_N, optimal_Fc);

% 使用找到的最优参数对信号进行最终滤波
Fc_normalized = optimal_Fc / Fnyq;
b = fir1(optimal_N, Fc_normalized, hamming(optimal_N+1));
